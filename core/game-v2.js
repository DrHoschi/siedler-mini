 ...
 * - Bitte nur EINEN Renderer laden (dieser). Nicht zusätzlich core.map.js etc.
...
(function(){
  ...
  function resizeToCssRect(){
    ...
    const cssW = Math.max(1, Math.floor(cssRect.width));
    const cssH = Math.max(1, Math.floor(cssRect.height));
    ...
    // Kein canvas.style.width/height setzen – die CSS-Größe kommt aus ui-layout.css

    // Erste Chance für initiales Fit, sobald Map bekannt ist:
  ...
  Game.start = function(map, opt={}){
    try {
      ...
      // Direkt passend einzoomen & zentrieren (entsprechend CSS-Rect)
  ...
  OK('Modul geladen (', 'v25.11.17-final', ')');
