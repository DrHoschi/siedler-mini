/* core/pathfinder.js — v16.5.2
   ...
   Fix: Start/Ziel dürfen „relaxed“ blockiert sein (keine Sackgasse mehr)
*/
(function(){
  'use strict';
  ...
  PF.invalidateRoads = function(){ /* Road-Cache wäre hier */ };
  ...
  // Relaxed-Blocker: Start/Ziel dürfen blockiert sein
  function mkIsBlockedRelaxed(sx,sy, tx,ty){
    return function(x,y){
  ...
  // --- Road-Connectivity (BFS 4-Nb auf RoadSet) ---
  ...
  function findPathRoads(sx,sy, tx,ty){
    var openH=0;
    var ox=new Int16Array(_w*_h), oy=new Int16Array(_w*_h);
    ...
    function push(x,y){ var id=idx(x,y); if(inOpen[id])return; inOpen[id]=1; ox[openH]=x; oy[openH]=y; openH++; }
    function pop(){
      var best=-1, bestF=1e9, bx=0,by=0;
      for (var i=0;i<openH;i++){
    ...
    while(true){
      ...
      for (var k=0;k<4;k++){
        ...
        var step=10, newG=og[idx(cur.x,cur.y)] + step;
        if (newG < og[id]){
          cameX[id]=cur.x; cameY[id]=cur.y; og[id]=newG; push(nx1,ny1);
        }
      }
    }
    return null;
  ...
  // --- A* Offroad (8-Nb + Diagonalen + Kosten-Bias) ---
  function findPathOffroad(sx,sy, tx,ty){
    var isBlocked = mkIsBlockedRelaxed(sx,sy,tx,ty);
    var cap=_w*_h;
    var og=new Int32Array(_w*_h); for(var i=0;i<og.length;i++) og[i]=1e9;
    var cameX=new Int16Array(_w*_h), cameY=new Int16Array(_w*_h);
    var inOpen=new Uint8Array(_w*_h), closed=new Uint8Array(_w*_h);
    ...
    function bestPop(){
      ...
      for (var i=0;i<qh;i++){
        ...
        var h = Math.max(dx,dy)*14; // Chebyshev grob
        var f = g+h;
