/* core/pathfinder.js — v16.5.1
   Hybrid-Pathfinding:
     - Straßen bevorzugt (4-Nachbarn)
     - Fallback Offroad mit Diagonalen (8-Nachbarn)
...
(function(){
  ...
  var _lastPaths = [];          // Pfade fürs Overlay
  ...
  PF.invalidateRoads = function(){ /* Platzhalter für Road-Cache, falls nötig */ };
  ...
  // ---------- A*: Offroad (8-Nachbarn, Diagonalen erlaubt) ----------
  function findPathOffroad(sx,sy, tx,ty){
    ...
    while(true){
      ...
      for (var k=0;k<dirs.length;k++){
        ...
        // Kosten-Bias: Road bevorzugt, Heat bevorzugt
        var costMul = 1.0;
  ...
  PF.drawOverlay = function(ctx, cam){
    ...
    for (var i=0;i<_lastPaths.length;i++){
      ...
      for (var k=0;k<path.length;k++){
        var x=path[k].x*tile + tile/2, y=path[k].y*tile + tile/2;
