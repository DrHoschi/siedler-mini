...
(function () {
  ...
  function neighborsRoad(x,y,out){
    for (var i=0;i<4;i++){
      ...
      if (!inb(nx,ny) || isBlocked(nx,ny) || !isRoad(nx,ny)) continue;
      out.push([nx,ny,1]);
    }
    return out;
  }
  ...
  function astar(sx,sy,tx,ty,mode){
    ...
    while(!open.empty() && iter++<maxIter){
      var cur=pop(), cx=cur.x, cy=cur.y, ci=idx(cx,cy);
      if (cx===tx && cy===ty){
        var path=[{x:tx,y:ty}];
        while(came[ci]!==-1){
  ...
  PF.init=function(getMapSize){
    ...
    }catch(e){ LOG('warn','[PF] init Fehler: '+(e&&e.message)); }
  };
  ...
  PF.applyHeat=function(path){
    ...
    _lastPaths.push(path); if (_lastPaths.length>6) _lastPaths.shift();
  };
  ...
  function tryLazyInit(){
    ...
    try{
      if (window.Game && typeof Game.getMapSize==='function'){
        PF.init(Game.getMapSize); _didLazyInit=true;
  ...
  PF.findPath=function(cfg){
    tryLazyInit();
    if (!cfg||!cfg.from||!cfg.to){ LOG('warn','[PF] findPath: ungültige Parameter'); return null; }
    if (!_w||!_h||!_heat){ LOG('warn','[PF] findPath ohne init() aufgerufen'); return null; }
    var sx=(cfg.from.x|0), sy=(cfg.from.y|0);
    var tx=(cfg.to.x|0),   ty=(cfg.to.y|0);
    var mode=cfg.mode||'auto';
    if (isBlocked(sx,sy)||isBlocked(tx,ty)){ LOG('warn','[PF] Start/Ziel blockiert'); return null; }
    var path=null;
    if (mode==='roads'){
      if (isRoad(sx,sy)&&isRoad(tx,ty)) path=astar(sx,sy,tx,ty,'roads');
    ...
    if (path&&path.length){ PF.applyHeat(path); return path; }
    LOG('warn','[PF] kein Pfad '+sx+','+sy+' → '+tx+','+ty); return null;
  };

  PF.drawOverlay=function(ctx, cam){
    try{
      if (!window.DEBUG_PATH_OVERLAY || !ctx || !_heat) return;
      ...
      for (var pI=0;pI<_lastPaths.length;pI++){
        var p=_lastPaths[pI]; if (!p||p.length<2) continue;
        ctx.beginPath();
        ...
        ctx.strokeStyle='rgba(0,128,255,0.9)'; ctx.stroke();
      }
    }catch(_){}
  };
