 ...
 * Version  : v26.08.31-sa04-continue-gate12
...
(function(){
  ...
  function prepareContinue(){
    ...
    if (!result?.ok){
      state.continuePrepared=false; state.userReady=false; state.mode=null;
  ...
  function onUserRequest(mode){
    if (state.started) return;
    state.userReady = true; state.mode = mode || 'new';
    INFO('UserReady ✓ via', state.mode);
    if (state.mode === 'continue' && state.saveV2Ready){ if (!prepareContinue()) return; }
    maybeStart();
    setTimeout(()=>{
      if (state.started) return;
  ...
  addEventListener('cb:assets-ready',(e)=>{ if(state.assetsReady)return; state.assetsReady=true; INFO('Assets bereit ✓',e?.detail||{}); maybeStart(); },{once:true});
  addEventListener('cb:registry:ready',(e)=>{ if(state.registryReady)return; state.registryReady=true; INFO('Registry bereit ✓',e?.detail||{}); maybeStart(); },{once:true});
  addEventListener('cb:savegame:v2:ready',(e)=>{ state.saveV2Ready=true; INFO('SaveGame V2 bereit ✓',e?.detail||{}); if(state.userReady&&state.mode==='continue'){if(!prepareContinue())return;} maybeStart(); },{once:true});
  ...
  appendScript('core/sa04.housing-menu.js?v=26.08.31-sa04-housing-menu1');
