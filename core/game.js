...
// Version : v18.5.0 (Gesten-Fix: Pinch/Pointer-Reset, iOS-Gesten blocken)
// ============================================================================
...
(() => {
  ...
  const _state = {
    ...
    hover:{x:-1,y:-1}, selectedBuilding:null, preview:null,
  ...
  // == Registry / Assets (wie gehabt, unverändert gekürzt) ===================
  ...
  // == Buildings / Platzierung (gekürzt, wie zuvor) ==========================
  function getBuildingDef(id){
    ...
    if(raw){
      ...
      if (Array.isArray(raw.entrances)&&raw.entrances.length) entrances=raw.entrances.map(e=>[(e[0]|0),(e[1]|0)]);
      else entrances=[[Math.floor((w-1)/2),h]];
  ...
  // == Rendering (wie zuvor; platzierte Gebäude ohne grünen Rahmen) ==========
  function frame(ts){
    if(!_state.started) return;
    const {ctx,canvas,map}=_state; if(!ctx||!canvas) return;
    const dt=_state.lastTs?Math.min(0.1,(ts-_state.lastTs)/1000):0; _state.lastTs=ts;
    map?.draw();
  ...
  function drawRect(ctx,sx,sy,w,h,ok){
    ...
    ctx.fillRect(sx,sy,w,h); ctx.strokeRect(sx+.5,sy+.5,w-1,h-1);
  }
  function drawPlacementSprite(ctx,id,sx,sy,wTiles,hTiles){
    const sprite=getImage(getSpriteUrl(id)); const s=_state.tile*_state.zoom;
    if (sprite) ctx.drawImage(sprite, sx, sy, wTiles*s, hTiles*s);
    else { const icon=getImage(getIconUrl(id)); if(icon) ctx.drawImage(icon, sx+4, sy+4, s-8, s-8); }
  }
    ...
    for (const p of _state.placements){
      ...
      if (_state.showEntrances){
        ...
        for (const {ex,ey} of entrancesAbs){
          ...
          ctx.strokeRect(epos.sx+pad+.5, epos.sy+pad+.5, s-2*pad-1, s-2*pad-1); ctx.restore();
        }
      }
    }
  }
  ...
  function drawGhost(ctx){
    ...
    const def=getBuildingDef(id); const s=_state.tile*_state.zoom; const pos=worldToScreen(gx*_state.tile, gy*_state.tile);
    ...
    emitPreviewEvent({ id, gx, gy, sx:pos.sx, sy:pos.sy, size:s, invalid:!ok,
      w:def.size.w, h:def.size.h, door:{...def.door}, entrances:def.entrances.slice(), entrancesAbs });
  }

  // == Map laden (wie gehabt) ================================================
  async function loadMap(mapId){
    try{
      if (typeof mapId==='string' && /\.json($|\?)/i.test(mapId)){
        const res=await fetch(mapId,{cache:'no-cache'}); if(!res.ok) throw new Error(`HTTP ${res.status}`);
      ...
      } else log('map: kein JSON → Default',{mapId});
  ...
  // --- Zentrale Aufräumer, um „stale“ Gesten zu verhindern ---
  ...
  function onPointerDown(ev){
    ...
    // iOS: falls 2. Finger dazukommt → Pinch starten
    if (_state.pointers.size===2 && tryStartPinch()) return;

    if (_state.selectedBuilding){
      // Baumodus: ein Finger = nur Ghost steuern (kein Pan)
  ...
  function onPointerMove(ev){
    ...
    // **Sicherheitsleine**: im Baumodus bei <=1 Finger niemals Pinch aktiv lassen
  ...
  function onPointerUp(ev){
    ...
    if (wasPlacing && !wasPinch && isTap){
      ...
      if (!ok){ _state.preview=null; emitPreviewEvent({invalid:true}); }
        ...
        const entrancesAbs=computeEntrancesAbs(gx,gy,def);
        emitPreviewEvent({ id:_state.preview.id, gx, gy, sx:pos.sx, sy:pos.sy, size:_state.tile*_state.zoom, invalid:false,
          w:def.size.w, h:def.size.h, door:{...def.door}, entrances:def.entrances.slice(), entrancesAbs });
      }
    }

    // Wenn kein Finger mehr drauf → alle Gesten sauber beenden
    if (_state.pointers.size===0){ _state.pinch.active=false; _state.panActive=false; }
  }

  // iOS/Touchpad: Wheel ignorieren (nur echte Maus)
  function onWheel(ev){
    // Auf Touch-Geräten keine Wheel-Zooms
  ...
  // Safari-spezifische Gesture-Events hart unterbinden (sonst Page-Zoom)
  ...
  function init(canvas){
    ...
    // iOS/Safari – Touch & Gesten strikt unterbinden (Page-Zoom etc.)
    ...
    // Map-Renderer
    if (typeof window.SiedlerMap!=='function'){ err('SiedlerMap fehlt – core/core.map.js nicht geladen?'); }
    else{
    ...
    window.addEventListener('pointercancel', onPointerUp);     // falls OS abbricht
    ...
    window.addEventListener('cb:build:select',(e)=>{
      ...
      _state.preview=null;
      clearPointers('select');          // ganz wichtig!
  ...
  async function start(mapId){
    ...
    onResize(); centerCameraOnMap();           // jetzt
    rebuildOccupied();
    ...
    // Sicherheitshalber noch einmal zentrieren, wenn Layout final ist
