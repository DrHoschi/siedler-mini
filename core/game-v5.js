/* ============================================================================
 * Datei   : core/game-v5.js
 * Projekt : Neue Siedler
 * Version : v25.11.27-prod-step2
...
(function(){
  ...
  // HINWEIS:
  // - IDs bitte an deine echten Gebäude-IDs anpassen (Registry / buildings.json).
  // - Wenn später Registry.production genutzt wird, hat diese Vorrang.
  ...
  // Produktions-Meta holen:
  // 1) falls Registry.production existiert (später)
  // 2) sonst HARDCODED_PRODUCTION
  function getProductionRuleFor(id){
    if (!id) return null;

    // 1) Registry-Integration (optional, bricht nie das Spiel)
    try {
      ...
      if (reg && typeof reg.get === 'function'){
        ...
        if (p){
          return {
            ...
            pickup   : p.pickup || null   // optionaler Abhol-Offset
          };
        }
      }
    } catch(e){
    ...
    // 3) Kleine Heuristik über den Namen (fallback)
  ...
  const S = {
    ...
    // Eintrag z. B.:
    // { id, x, y, w, h, stock:{wood:2}, _prodTimer:3.2 }
    buildings: [],

    // HQ-Info (für Lieferjobs)
    hqPos   : null,   // {x,y} in Tiles
    hqStock : {},     // zentrale Ressourcen (falls benötigt)
  ...
  function drawBuildings(){
    ...
    for (const b of S.buildings){
      ...
      // Gebäude-Körper (Sprite oder Platzhalter)
      if (spr && spr.complete){
      ...
      if (b.stock){
        ...
        if (keys.length){
          ctx.save();
          ctx.setTransform(1,0,0,1,0,0); // HUD-Koords
          const ts = S.tileW;
          const sx = (b.x * ts) + 4;
          const sy = (b.y * ts) + 4;
  ...
  function updateProduction(dt){
    ...
    // HQ-Position bei Bedarf nachziehen (erste gefundene HQ)
    if (!S.hqPos){
    ...
    for (const b of S.buildings){
      ...
      if (!rule) continue; // Gebäude produziert nichts

      // Produktions-Timer hochzählen
      ...
      // 2) Träger-Job erzeugen (FROM = Gebäude, TO = HQ oder Gebäude selbst)
      const from = {
        x: (rule.pickup?.x ?? b.x),
        y: (rule.pickup?.y ?? b.y)
      };
      ...
      try {
        // via Wrapper auf GameUnits
        window.Game.addJob?.(job);
      } catch(e){
        WARN('addJob fail', e?.message||e);
      }

      // 3) Debug-/Inspector-Event
      try {
  ...
  /* ==========================================================================
   * 4b) FRAME-LOOP – Map + Gebäude + Overlays + Units-Tick
   * ======================================================================== */
  function frame(){
    if (!S.running) return;

    const dt = 1/60; // vereinfachtes dt (60 FPS Target)

    // 1) Produktionssystem ticken (Gebäude → Jobs erzeugen)
    updateProduction(dt);

    // 2) Units / Träger-System pro Frame ticken lassen
    window.dispatchEvent(new CustomEvent('cb:game:tick', {
      detail: { dt }
    }));

    // 3) Map + Gebäude mit Kamera-Transform zeichnen
    clear();
    applyCamera();
    drawLayersCulled();
    drawBuildings();

    // 4) Overlays (Träger-Punkte, Trampelpfade) zeichnen
    ...
    // 5) Render-Event für weitere Overlays / Debug
    try {
  ...
  function placeInternal(id, x, y, opt = {}) {
    ...
    const b = {
      ...
      w,
      h,
      stock: {},      // Lager (resId → Menge)
      _prodTimer: 0   // Produktions-Timer (Sekunden)
    };

    S.buildings.push(b);

    // HQ-Position merken (falls dieses Gebäude das HQ ist)
    if (!S.hqPos && id === 'b.hq'){
      S.hqPos = { x:bx, y:by };
    }
    ...
    // Info an Träger-System: Es gibt ein neues Gebäude (evtl. Baustelle / Produktionsort)
    window.dispatchEvent(
  ...
  // von carrier.js genutzte Hooks: takeFromBuilding / deliverToHQ
  ...
  Game.deliverToHQ = function(res,qty){
    ...
    // HUD / Ressourcen-System informieren
    try {
      window.dispatchEvent(new CustomEvent('cb:res:change', {
        detail: { res, delta:qty, source:'hq' }
      }));
    } catch(e){
      // ignorieren – Fallback wäre CarrierRuntime ohne Game.deliverToHQ
    }
    return true;
  };
  ...
  OK('Modul geladen (v25.11.27-prod-step2)');
