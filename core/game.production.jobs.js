...
(function () {
  ...
  function enqueueCarryJobsForOutput(detail) {
    ...
    // ------------------------------------------------------------
    // D5 (optional): Output zuerst als Stock am Gebäude puffern
    // ------------------------------------------------------------
    // Aktiv, wenn BuildingStock existiert UND:
    //   - detail.store === true  (explizit) ODER
    //   - BuildingKind in DEFAULT_KINDS liegt (Holz/Stein/Fisch Gebäude)
    // Dadurch können wir später z.B. "Pickup-Jobs" limitieren und
    // das klassische Siedler-Verhalten (Gebäude puffert → Träger holt ab)
    // sauber abbilden.
    try{
      const BS = window.BuildingStock;
      const wantStore = !!(detail && (detail.store === true || detail.mode === 'stock'));
      const kind = String(building.kind || detail.kind || detail.id || '');
      const autoStore = BS && typeof BS.isKindStockable === 'function' ? BS.isKindStockable(kind) : false;

      if (BS && typeof BS.add === 'function' && BS.isEnabled && BS.isEnabled() && (wantStore || autoStore)){
        BS.add(building, resId, qty);
        return; // KEINE direkten Carry-Jobs hier
      }
    }catch(e){
      // falls Stock-Modul fehlt/fehlerhaft: einfach normal weiter
      WARN('Stock-Route Fehler – fallback zu direkten Carry-Jobs', e);
    }
