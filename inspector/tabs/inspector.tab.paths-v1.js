...
(function () {
  function renderPathsTab(sectionEl) {
    sectionEl.innerHTML = [
      ...
      '<div class="row" style="margin-top:10px;align-items:center;gap:8px;">',
      '  <label style="min-width:110px;opacity:.85">Softness</label>',
      '  <input type="range" data-action="softness" min="50" max="200" step="5" value="100" style="flex:1">',
      '  <span data-action="softness-label" style="min-width:62px;text-align:right;opacity:.8">100%</span>',
      '</div>',
      '<div class="row" style="margin-top:10px;align-items:center;gap:8px;">',
    ...
    // Softness Slider (weiche Kanten / Halo)
    const ss = sectionEl.querySelector('[data-action="softness"]');
    const ssLabel = sectionEl.querySelector('[data-action="softness-label"]');
    ss?.addEventListener('input', () => {
      const percent = Number(ss.value) || 100;
      const mult = percent / 100;
      if (ssLabel) ssLabel.textContent = `${percent}%`;
      send('cb:path:softness', { percent, softnessMult: mult });
      try{ window.PathOverlay?.setSoftnessMult?.(mult); }catch(_){/*noop*/}
    });
    ...
    try{
      ...
      if (st && ss){
        const pct = Math.round(Number(st.softnessMult || 1) * 100);
        ss.value = String(Math.max(50, Math.min(200, pct)));
        if (ssLabel) ssLabel.textContent = `${ss.value}%`;
      }
%`;
      }
    }catch(_){/*noop*/}
...
(() => {
  ...
  function mount(panel){
    panel.innerHTML = `
      ...
      <div class="pad" style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
        <label style="opacity:.85">Softness</label>
        <input id="p-softness" type="range" min="50" max="200" step="5" value="100" style="min-width:200px;flex:1">
        <span id="p-softness-label" class="muted" style="min-width:62px;text-align:right">100%</span>
      </div>
</div>
      <div class="pad" style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
    ...
    const slSoft       = panel.querySelector('#p-softness');
    const slSoftLabel  = panel.querySelector('#p-softness-label');
    ...
    function renderState(st){
      ...
      if (slSoft && st?.softnessMult != null){
        const pct = Math.round(Number(st.softnessMult) * 100);
        slSoft.value = String(Math.max(50, Math.min(200, pct)));
        if (slSoftLabel) slSoftLabel.textContent = `${slSoft.value}%`;
      }
%`;
      }
    }
    ...
    // Softness Slider
    slSoft?.addEventListener('input', () => {
      const percent = Number(slSoft.value) || 100;
      const mult = percent / 100;
      if (slSoftLabel) slSoftLabel.textContent = `${percent}%`;
      send('cb:path:softness', { percent, softnessMult: mult });
    });
