...
// ------------------------------------------------------------
// Auto-Discovery: Atlanten werden oft erst *nach* async Asset-Load verfügbar
// (z. B. wenn der Inspector sehr früh geöffnet wird oder iOS/Safari Caches zickt).
// Wir versuchen daher nach dem Mount automatisch ein paar Mal zu refreshen.
// ------------------------------------------------------------
let _autoTry = 0;
let _autoTimer = null;
function stopAutoAtlasDiscover(){
  if (_autoTimer){
    clearInterval(_autoTimer);
    _autoTimer = null;
  }
}
function startAutoAtlasDiscover(){
  stopAutoAtlasDiscover();
  _autoTry = 0;
  _autoTimer = setInterval(() => {
    _autoTry++;
    const names = listAtlasNames();
    if (names && names.length){
      // Falls noch nichts gewählt ist (oder alter Key nicht mehr existiert): ersten nehmen
      if (!state.atlasName || !names.includes(state.atlasName)){
        state.atlasName = names[0];
        try{ atlasSel.value = state.atlasName; }catch(_e){}
      }
      repopulate();
      stopAutoAtlasDiscover();
      return;
    }
    // Hard stop (damit nix ewig läuft)
    if (_autoTry >= 25) stopAutoAtlasDiscover();
  }, 250);
}

// Hook: Wenn dein Loader ein Custom-Event feuert (bei dir existiert es teils schon),
// refreshen wir sofort nochmal.
try{
  window.addEventListener('cb:assets-ready', () => { repopulate(); startAutoAtlasDiscover(); });
  document.addEventListener('cb:assets-ready', () => { repopulate(); startAutoAtlasDiscover(); });
}catch(_e){}
