...
(function () {
  ...
  function btn(label, attrs) {
    var b = el("button", { "class": "cb-btn" }, label);
    if (attrs) for (var k in attrs) if (attrs.hasOwnProperty(k)) b[k] = attrs[k];
    return b;
  }
  ...
  window.GameUI.openStartPanel = function (opts) {
    ...
    try {
      var last = localStorage.getItem("cb:lastMap");
    ...
    var cacheBtn = btn("🧹 Cache-Booster", { onclick: function(){ location.href = location.href.split("?")[0] + "?v=" + Date.now(); }});
