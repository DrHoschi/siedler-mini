 ...
 * Version  : v25.11.09-final+size3.1
 * Modul    : Baumenü (Build-Dock) – Kategorien + Kartenraster
 *
 * Änderungen ggü. vorher:
 *   – Beispiel-HUD-HTML als Kommentar (war vorher "nackt" im JS → Syntaxfehler)
 *   – Sonst nur Kommentare/kleine Klarstellungen, Logik unverändert
 *
 * Kernfunktion:
 *   – Liest Gebäude aus Registry oder data/buildings.json
 *   – Baut Baumenü: Kategorien + Kartenraster
 *   – Jede Karte trägt data-w / data-h (Fallback 3x3) für das Platzier-Tool
 *   – Öffnen/Schließen via Button #btn-build oder Events cb:build:open/close
 * ========================================================================== */

/* ============================================================================
 * HINWEIS: Beispiel für eine HUD-Zelle (NUR DOKU, NICHT im JS ausführen)
 *
 *   <div class="hud__cell">
 ...
 *   </div>
 *
 * Dieses Snippet gehört ins HTML / in das HUD-Template, NICHT in die JS-Datei.
 * ========================================================================== */

(function EnsureDock(){
  /* -------------------------------------------------------------------------
   * Failsafe: Stelle sicher, dass #build-dock existiert.
   * Falls im HTML vergessen, wird es hier im <body> angelegt.
   * ---------------------------------------------------------------------- */
  ...
  if (!el){
    ...
    el.id = 'build-dock';
    el.hidden = true;
    el.style.overflow = 'auto';   // Scrollen erlauben, falls Inhalt größer als Dock
...
(function(){
  ...
  if (!$dock){
    ERR('DOM: #build-dock fehlt (EnsureDock ist evtl. nicht gelaufen?)');
    return;
  }
  ...
  /* ----------------------------- Modul-Status ---------------------------- */
  let BUILDINGS   = [];           // Normalisierte Gebäudedaten
  let CATEGORIES  = [];           // {id,name,count}
  let ACTIVE_CAT  = 'all';        // aktuell ausgewählte Kategorie
  let IS_OPEN     = false;
  let INIT_DONE   = false;
  ...
  const byCat   = (list, cat) => (cat === 'all')
    ? list
    : list.filter(b => (b.categories||[]).includes(cat));

  /**
   * Rohdaten → Normalisierte Gebäudedaten
   * - sorgt dafür, dass w/h IMMER gesetzt sind (Fallback 3x3)
   * - cost wird in einheitliches Array [{id,amount}] umgewandelt
   */
  function normalizeBuilding(raw){
    ...
    if (Array.isArray(raw.cost)) {
      cost = raw.cost
        .map(c => ({ id: String(c.id), amount: Number(c.amount||0) }))
        .filter(c => c.id && c.amount > 0);
    } else if (raw.cost && typeof raw.cost === 'object') {
      cost = Object.keys(raw.cost)
        .map(k => ({ id:String(k), amount:Number(raw.cost[k]||0) }))
        .filter(c => c.amount > 0);
    }
  ...
  async function loadBuildings(){
    try {
      ...
      WRN('Registry.list("building") Fehler:', e);
    }
  ...
  /** Kategorien aus den Gebäuden ableiten */
  ...
  /** Grundgerüst des Docks in #build-dock schreiben */
  function renderDockSkeleton(){
    ...
    $dock.querySelector('#build-close')
      ?.addEventListener('click', closeDock);
  }

  /** Kategorienleisten-Buttons rendern */
  function renderCategories(){
    ...
    CATEGORIES.forEach(cat => {
      ...
      btn.innerHTML = `
        <span class="build-cat__name">${cat.name}</span>
        <span class="build-cat__cnt">${cat.count}</span>
      `;
  ...
  async function initAndRender(){
    ...
    try {
      // Dock zu Beginn unsichtbar vorbereiten
      $dock.hidden = false;
      $dock.classList.add('hidden');
      $dock.style.display = 'none';
  ...
  function wireUI(){
    ...
    if (btn.dataset.wiredBuild === '1'){
      INF('Button bereits verdrahtet – übersprungen');
      return;
    }
  ...
  /* --------------------------- Event-Hooks ------------------------------- */
  // UI-Fertig → Button verdrahten
  window.addEventListener('cb:ui-ready',       wireUI,        { once:true });
  // Assets/Registry/Game → Baumenü einmalig initialisieren
