/* ================================================================================================
   ...
   Version: v16.1.3-merge1
   Zweck:
   - Canvas/Rendering-Helfer (HiDPI, crisp/smooth)
   - Loader (JSON/PNG), kleiner AssetManager
   - Carry-System-Utilities (Attach-Offsets, Z-Order, Debug)
...
// Version/Token für Cache-Busting (auch für Characters genutzt)
const VERSION = "16.1.3-merge1";
const TOKEN   = `${VERSION}-${Date.now().toString(36)}`;

// ---------------------------------------------------------
// Helpers (Canvas/Context/Loader) — bestehend + erweitert
// ---------------------------------------------------------
...
function coerceCanvasAndCtx(target){
  ...
  try{
    if (typeof HTMLCanvasElement !== 'undefined' && target instanceof HTMLCanvasElement) {
      cv = target;
      ctx = target.getContext && target.getContext('2d') || null;
...
/** Versionstoken an URLs hängen (gegen hartnäckiges Caching auf iOS/Safari). */
...
export async function loadImage(url){
  // Progressive: erst fetch → blob (für createImageBitmap), fällt zurück auf <img>
  const res = await fetch(withToken(url));
  if(!res.ok) throw new Error(`HTTP ${res.status} for ${url}`);
...
// ---------------------------------------------------------
// Charakter-Sprite/Atlas – Pfade & Lazy-Loader (High-End 2048)
// Lege Dateien ab unter:
...
function drawChar(ctx, name, dir, x, y){
  if(!_characters || !_characters.image || !_characters.atlas){
    ...
    // Notfall: Lazy-Load starten; diese erste Zeichnung kann leer bleiben.
  ...
  // imageBitmap oder HTMLImage kompatibel zeichnen
  if ('close' in _characters.image) {
    // ImageBitmap
    const canvas = ctx.canvas;
    // drawImage akzeptiert ImageBitmap transparent identisch
...
// ---------------------------------------------------------
// Exports (Default-Bündel + named Exports)
// ---------------------------------------------------------
const Assets = {
  // Canvas/Loader
  setupHiDPICanvas,
