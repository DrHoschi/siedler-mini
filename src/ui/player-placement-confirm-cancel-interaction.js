...
function requireRuntime(runtime) { if (!runtime?.map || !runtime?.domains || typeof runtime.renderCurrentWorld !== 'function' || !runtime.runtime || typeof runtime.runtime.state !== 'string') throw new TypeError('current authoritative runtime boundary required'); return runtime; }
...
export function createPlayerPlacementConfirmCancelInteraction({ placementController, runtime } = {}) {
  ...
  function confirm() {
    ...
    if (currentRuntime.runtime.state !== 'RUNNING') {
      lastResult = deepFreeze({ kind: 'player-placement-confirm-result', status: 'REJECTED', reason: 'RUNTIME_NOT_RUNNING', commitResult: null }); return lastResult;
    }
...
export function installIM16EBrowserControls() {
  ...
  function sync() { const state = placementController.getState(); const active = state?.status === 'ACTIVE'; const running = runtime.runtime?.state === 'RUNNING'; confirmButton.disabled = !active || !state.targetCellId || !running; cancelButton.disabled = !active; confirmButton.dataset.targetCellId = state?.targetCellId ?? ''; return state; }
