...
export function runCr14cSelfTest() {
  ...
  const movementA=CarrierMovementContract.define({unitId:carrierA,currentPosition:{x:0,y:0},state:'IDLE',targetPosition:null});
  const movementB=CarrierMovementContract.define({unitId:carrierB,currentPosition:{x:0,y:1},state:'IDLE',targetPosition:null});

  check('free-cell-single-carrier-enters',()=>{
    const result=OccupancyAwareMovementIntegration.advance({route,movement:movementA,nextCellOccupancy:CellOccupancyContract.define(),contenderCarrierIds:[carrierA],maxDistance:1});
  ...
  check('occupied-by-other-carrier-causes-wait',()=>{
    const result=OccupancyAwareMovementIntegration.advance({route,movement:movementA,nextCellOccupancy:CellOccupancyContract.define({state:'OCCUPIED',carrierId:carrierB}),contenderCarrierIds:[carrierA],maxDistance:1});
  ...
  check('arbitration-winner-enters',()=>{
    const result=OccupancyAwareMovementIntegration.advance({route,movement:movementA,nextCellOccupancy:CellOccupancyContract.define(),contenderCarrierIds:[carrierB,carrierA],maxDistance:1});
    return result.allowed===true&&result.arbitration?.winnerCarrierId===carrierA&&result.movement.currentPosition.x===1;
  });
  ...
  check('arbitration-loser-waits',()=>{
    ...
    const result=OccupancyAwareMovementIntegration.advance({route:routeB,movement:movementB,nextCellOccupancy:CellOccupancyContract.define(),contenderCarrierIds:[carrierB,carrierA],maxDistance:1});
    return result.allowed===false&&result.waiting===true&&result.arbitration?.winnerCarrierId===carrierA&&result.movement===movementB;
  });

  check('one-check-never-crosses-more-than-next-route-cell',()=>{
    const result=OccupancyAwareMovementIntegration.advance({route,movement:movementA,nextCellOccupancy:CellOccupancyContract.define(),contenderCarrierIds:[carrierA],maxDistance:10});
    return result.movement.currentPosition.x===1&&result.movement.currentPosition.y===0;
  });

  check('same-input-is-deterministic',()=>{
    const args={route,movement:movementA,nextCellOccupancy:CellOccupancyContract.define(),contenderCarrierIds:[carrierB,carrierA],maxDistance:1};
