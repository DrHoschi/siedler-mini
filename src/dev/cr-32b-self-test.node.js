...
const UNIT_1 = 'unit:00000001';
const UNIT_2 = 'unit:00000002';
const UNIT_3 = 'unit:00000003';
...
function executeRealStep({ map, carrierId, currentCell, nextCell, targetCell, stepNumber }) {
  const route = Object.freeze({ startPosition: Object.freeze({ ...currentCell }), targetPosition: Object.freeze({ ...targetCell }), waypoints: Object.freeze([Object.freeze({ ...nextCell })]), state: 'ACTIVE' });
...
function pathStep(map, carrierId = UNIT_1, stepNumber = 1) { return executeRealStep({ map, carrierId, currentCell: { x: 0, y: 0 }, nextCell: { x: 1, y: 0 }, targetCell: { x: 2, y: 0 }, stepNumber }); }
function roadStep(map, carrierId = UNIT_1, stepNumber = 1) { return executeRealStep({ map, carrierId, currentCell: { x: 1, y: 0 }, nextCell: { x: 2, y: 0 }, targetCell: { x: 3, y: 0 }, stepNumber }); }
function neutralStep(map, carrierId = UNIT_1, stepNumber = 1) { return executeRealStep({ map, carrierId, currentCell: { x: 1, y: 1 }, nextCell: { x: 0, y: 1 }, targetCell: { x: 0, y: 0 }, stepNumber }); }

{
  ...
  assert.equal(first.status, 'ACCUMULATED'); assert.equal(first.traversalType, 'PATH'); assert.equal(first.usageCount, 1); assert.equal(first.wearUnits, 1); assert.equal(wear.at({ x: 1, y: 0 }).wearUnits, 1);
  console.log('PASS real-cr21c-completed-step-accumulates-path-usage-and-wear');
}
{
  const { map, wear } = makeFixture();
  wear.recordCompletedStep(roadStep(map, UNIT_1, 10));
  const second = wear.recordCompletedStep(roadStep(map, UNIT_2, 20));
  assert.equal(second.traversalType, 'ROAD'); assert.equal(second.usageCount, 2); assert.equal(second.wearUnits, 2);
  console.log('PASS repeated-real-completed-steps-accumulate-road-wear');
}
{
  const a = makeFixture(); const b = makeFixture();
  const aSequence = [pathStep(a.map, UNIT_1, 1), roadStep(a.map, UNIT_2, 2), pathStep(a.map, UNIT_3, 3)];
  const bSequence = [pathStep(b.map, UNIT_1, 1), roadStep(b.map, UNIT_2, 2), pathStep(b.map, UNIT_3, 3)];
  for (const step of aSequence) a.wear.recordCompletedStep(step); for (const step of bSequence) b.wear.recordCompletedStep(step);
...
{
  const { map, wear } = makeFixture(); const ignored = wear.recordCompletedStep(neutralStep(map));
  assert.equal(ignored.status, 'IGNORED'); assert.equal(ignored.reason, 'NEUTRAL_CELL'); assert.equal(wear.entries().length, 0);
  console.log('PASS neutral-cell-does-not-accumulate-path-road-wear');
}
{
  const { wear } = makeFixture();
  assert.throws(() => wear.recordCompletedStep({ kind: 'reservation-controlled-step-movement', status: 'PLANNED', carrierId: UNIT_1, enteredCell: { x: 1, y: 0 } }), /COMPLETED/);
  assert.throws(() => wear.recordCompletedStep({ kind: 'route', status: 'COMPLETED', carrierId: UNIT_1, enteredCell: { x: 1, y: 0 } }), /CR-21C/);
  assert.throws(() => wear.recordCompletedStep({ kind: 'reservation-controlled-step-movement', status: 'COMPLETED', carrierId: UNIT_1, enteredCell: { x: 1, y: 0 }, lifecycleState: { status: 'GRANTED' } }), /consume/);
  assert.equal(wear.entries().length, 0); console.log('PASS route-reservation-or-planned-movement-alone-cannot-create-wear');
}
{
  const { map, wear, classification } = makeFixture(); const beforePathCost = RoadPreferenceCostPolicy.resolve('PATH').traversalCost; const beforeRoadCost = RoadPreferenceCostPolicy.resolve('ROAD').traversalCost;
  wear.recordCompletedStep(pathStep(map, UNIT_1, 30)); wear.recordCompletedStep(roadStep(map, UNIT_2, 31));
  assert.equal(classification.classAt({ x: 1, y: 0 }), 'PATH'); assert.equal(classification.classAt({ x: 2, y: 0 }), 'ROAD'); assert.equal(RoadPreferenceCostPolicy.resolve('PATH').traversalCost, beforePathCost); assert.equal(RoadPreferenceCostPolicy.resolve('ROAD').traversalCost, beforeRoadCost);
