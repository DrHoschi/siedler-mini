import { RuntimeConfig } from './runtime/config.js?v=im18d-1';
...
if(testEl){testEl.textContent=`IM-18D · TESTBUILD 1 — Production Requirement / Recipe Integration · existing deterministic input/output recipe bound to assigned operational Building · no execution / consumption / output creation · Population ${initialOwners.housingPopulation.population.count} · Gold ${initialOwners.goldSettlement.state.balance} · ${initialRender.projection.buildings.length} Buildings / ${initialRender.projection.persons.length} Persons`;testEl.dataset.pass='pending';}
...
document.title='Neue Siedler – IM-18D Production Requirement / Recipe Integration';
const verificationTitle=document.querySelector('.verification-card h1'); if(verificationTitle) verificationTitle.textContent='IM-18D – Production Requirement / Recipe Integration';
const verificationText=document.querySelector('.verification-card p'); if(verificationText) verificationText.textContent='IM-18D binds the frozen IM-18C assigned operational Building to the existing deterministic ProductionBuildingStock recipe contract. Input/output resource-type requirements are preserved through the existing BuildingStock production boundary. No production execution, input consumption or output creation occurs.';
const surfaceNote=document.querySelector('.surface-note'); if(surfaceNote) surfaceNote.textContent='IM-18D · TESTBUILD 1 · Production recipe integration only · no execution / consumption / output creation';
console.info('[IM-18D] Production Requirement / Recipe Integration',{build:RuntimeConfig.build,scenarioId:BASELINE_MINIWORLD_SCENARIO_ID,frozenIM18CWorkforceAssignmentReused:true,existingProductionBuildingStockRecipeReused:true,productionExecutionAdded:false,inputConsumptionAdded:false,outputCreationAdded:false});
...
void import('./domain/operational-building-workforce-assignment-integration.js?v=im18c-1');
void import('./domain/operational-building-production-recipe-integration.js?v=im18d-1');
