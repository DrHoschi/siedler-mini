...
const map=new MapStructure(world,{name:'CR-06B Pickup Execution Foundation',width:8,height:8,cellSize:1,metadata:{foundation:'CR-06B-PICKUP-EXECUTION-FOUNDATION'}});
...
const reports={
  ...
  cr03Freeze:runCr03FreezeGate(),cr04a:runCr04aSelfTest(),cr04b:runCr04bSelfTest(),cr04c:runCr04cSelfTest(),cr04Freeze:runCr04FreezeGate(),cr05a:runCr05aSelfTest(),cr05b:runCr05bSelfTest(),cr05c:runCr05cSelfTest(),cr05Freeze:runCr05FreezeGate(),cr06a:runCr06aSelfTest(),cr06b:runCr06bSelfTest()
};
const failedLayers=Object.entries(reports).filter(([,r])=>!r.pass||('blockerCount'in r&&r.blockerCount!==0)).map(([n])=>n);
const cr06bFailures=reports.cr06b.results?.filter(r=>!r.pass).map(r=>r.error?`${r.name}: ${r.error}`:r.name)??[];
const pass=failedLayers.length===0;
if(testEl)testEl.textContent=pass?'CR-06B PICKUP EXECUTION FOUNDATION: PASS / 0 BLOCKER':`CR-06B PICKUP EXECUTION: FAIL — ${[...failedLayers,...cr06bFailures].join(' | ')}`;
window.CleanRuntime=Object.freeze({config:RuntimeConfig,runtime,renderer,world,map,domains,resources,resourceClaims,resourceDemands,resourceMatching,resourceAssignment,reports,selfTest:()=>runCr06bSelfTest()});
console.info('[CR-06B] Pickup Execution Foundation',{build:RuntimeConfig.build,cr03Regression:reports.cr03Freeze,cr04Regression:reports.cr04Freeze,cr05Regression:reports.cr05Freeze,cr06aRegression:reports.cr06a,cr06b:reports.cr06b,failedLayers,cr06bFailures,overallPass:pass});
