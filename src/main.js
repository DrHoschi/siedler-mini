import { RuntimeConfig } from './runtime/config.js?v=im18c-1';
...
if(testEl){testEl.textContent=`IM-18C · TESTBUILD 1 — Deterministic Workforce Assignment Integration · eligible FREE persons assigned deterministically · duplicate assignment blocked · no production · Population ${initialOwners.housingPopulation.population.count} · Gold ${initialOwners.goldSettlement.state.balance} · ${initialRender.projection.buildings.length} Buildings / ${initialRender.projection.persons.length} Persons`;testEl.dataset.pass='pending';}
...
document.title='Neue Siedler – IM-18C Deterministic Workforce Assignment';
const verificationTitle=document.querySelector('.verification-card h1'); if(verificationTitle) verificationTitle.textContent='IM-18C – Deterministic Workforce Assignment Integration';
const verificationText=document.querySelector('.verification-card p'); if(verificationText) verificationText.textContent='IM-18C consumes the frozen IM-18B workforce requirement/eligibility result, re-checks current workforce state, and assigns the required number of eligible FREE persons deterministically through the existing workforce assignment-state authority. Already assigned or unavailable persons cannot be double-assigned. No production is executed.';
const surfaceNote=document.querySelector('.surface-note'); if(surfaceNote) surfaceNote.textContent='IM-18C · TESTBUILD 1 · Deterministic workforce assignment · duplicate assignment blocked · no production';
console.info('[IM-18C] Deterministic Workforce Assignment Integration',{build:RuntimeConfig.build,scenarioId:BASELINE_MINIWORLD_SCENARIO_ID,frozenIM18BEligibilityReused:true,deterministicAssignmentAdded:true,existingWorkforceStateAuthorityReused:true,duplicateAssignmentPrevented:true,productionAdded:false});
