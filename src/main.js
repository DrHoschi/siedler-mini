import { RuntimeConfig } from './runtime/config.js?v=im17g-1';
...
if(testEl){testEl.textContent=`IM-17G — Player Construction State Projection — Runtime foundation loaded — Player UI may display only authoritative construction requirement, progress and frozen-IM-17F completion state · Population ${initialOwners.housingPopulation.population.count} · Gold ${initialOwners.goldSettlement.state.balance} · ${initialRender.projection.buildings.length} Buildings / ${initialRender.projection.persons.length} Persons`;testEl.dataset.pass='pending';}
...
document.title='Neue Siedler – IM-17G Player Construction State Projection';
const verificationTitle=document.querySelector('.verification-card h1'); if(verificationTitle) verificationTitle.textContent='IM-17G – Player Construction State Projection';
const verificationText=document.querySelector('.verification-card p'); if(verificationText) verificationText.textContent='Player UI projects only authoritative economic construction state: waiting for material from existing requirement/progress truth, under construction from existing progress, and completed only from frozen IM-17F completion. UI owns no admission, demand, resource, delivery, progress or completion truth.';
const surfaceNote=document.querySelector('.surface-note'); if(surfaceNote) surfaceNote.textContent='IM-17G Player Projection ← existing Requirement + Progress + frozen IM-17F Completion · read-only UI truth projection';
console.info('[IM-17G] Player Construction State Projection runtime foundation',{build:RuntimeConfig.build,scenarioId:BASELINE_MINIWORLD_SCENARIO_ID,authoritativeRequirementProjection:true,authoritativeProgressProjection:true,frozenIM17FCompletionProjection:true,uiTruthAuthority:false,workforceIntegration:false,productionIntegration:false,wholeBlockGateExecuted:false});

void import('./ui/player-placement-confirm-cancel-interaction.js?v=im17g-1');
void import('./ui/player-building-selection-placement-activation.js?v=im17g-1');
void import('./ui/authoritative-construction-result-player-ui-projection.js?v=im17g-1');
void import('./domain/player-construction-runtime-admission-contract.js?v=im17g-1');
void import('./domain/economic-construction-requirement-contract.js?v=im17g-1');
void import('./domain/player-placement-construction-initialization-integration.js?v=im17g-1');
void import('./domain/construction-demand-existing-logistics-integration.js?v=im17g-1');
void import('./domain/delivered-material-construction-progress-settlement.js?v=im17g-1');
void import('./domain/construction-completion-integration.js?v=im17g-1');
void import('./ui/player-construction-state-projection.js?v=im17g-1');
void import('./im17g-runtime-evidence.js?v=im17g-1');
