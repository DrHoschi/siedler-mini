/* Siedler‑Mini V14.7‑hf2 — game.js (Start/Loop + Map‑Loader) */

async function tryImport(path){ try{ return await import(path); } catch(e){ console.error(`Import fehlgeschlagen: ${path}`, e); return null; } }
...
function drawHUD(ctx, lines, dpr){
  const pad=HUD_PAD*dpr, w=ctx.canvas.width, h=ctx.canvas.height;
  ctx.save(); ctx.globalAlpha=.85; ctx.fillStyle='#0f1b29'; ctx.fillRect(pad,pad,Math.min(560*dpr,w-2*pad),20*dpr*(lines.length+1)); ctx.restore();
...
class Game{
  constructor({canvas,ctx}){ this.cv=canvas; this.ctx=ctx; this.dpr=1; this.camera={x:0,y:0,zoom:1};
    ...
    this.state={frames:0,t0:performance.now(),last:performance.now(), map:null, assets:null}; this.onResize=()=>{ this.dpr=setupHiDPICanvas(this.cv); }; }
  async init(){
  ...
  async tryLoadMap(){
    if(!M_TOOLS || !M_TOOLS.SiedlerMap){ console.warn('tools/map-runtime.js fehlt — überspringe Kartenladung.'); return; }
    ...
    try{
      ...
      await world.loadFromObject(json); this.state.map=world;
  ...
  render(dt){
    ...
    if(this.state.map && this.state.map.draw){
      ctx.save(); this.state.map.draw(ctx, {x:camera.x,y:camera.y,w:cv.width/dpr,h:cv.height/dpr, zoom:camera.zoom}, performance.now()-this.state.t0); ctx.restore();
